...
class TSCRunner(BaseRunner):
    '''
    Register TSCRunner for traffic signal control tasks.
    '''
    def __init__(
        self,
        logger,
        ...
        self.test_interval = Registry.mapping['trainer_mapping']['setting'].param.get('test_interval', 10)  # 默认每10个episode测试一次
    ...
    def train(self):
        ...
        flush = 0
        # 添加episode级别的进度条
        episode_pbar = tqdm(range(self.episodes), desc="训练进度", unit="episode")
        for e in episode_pbar:
            ...
            i = 0
            # 添加step级别的进度条
            step_pbar = tqdm(total=self.steps, desc=f"Episode {e+1}/{self.episodes}", 
            ...
            episode_pbar.set_postfix({
                'travel_time': f'{self.metric.real_average_travel_time():.1f}',
                'reward': f'{self.metric.rewards():.2f}',
                'queue': f'{self.metric.queue():.2f}',
            ...
            # 显存监控 - 每个episode输出一次
            allocated_gb, reserved_gb = get_gpu_memory_info()
            self.logger.info("step:{}/{}, q_loss:{}, rewards:{}, queue:{}, delay:{}, throughput:{}, GPU_mem:{:.2f}GB/{:.2f}GB".format(
                i, self.steps, mean_loss, self.metric.rewards(), self.metric.queue(), 
                self.metric.delay(), int(self.metric.throughput()), allocated_gb, reserved_gb))
            ...
            # 每隔test_interval个episode测试一次
            if self.test_when_train and (e % self.test_interval == 0 or e == self.episodes - 1):
                self.train_test(e)
        episode_pbar.close()  # 关闭episode进度条
        # self.dataset.flush([ag.replay_buffer for ag in self.agents])
    ...
    def train_test(self, e):
        ...
        for a in self.agents:
            a.reset()
        test_pbar = tqdm(range(self.test_steps), desc=f"测试 Episode {e}", unit="step", leave=False)
        for i in test_pbar:
            if i % self.action_interval == 0:
    ...
    def test(self, drop_load=True):
        ...
        for a in self.agents:
            a.reset()
        final_test_pbar = tqdm(range(self.test_steps), desc="最终测试", unit="step")
        for i in final_test_pbar:
            if i % self.action_interval == 0:
                phases = np.stack([ag.get_phase() for ag in self.agents])
                actions = []
                for idx, ag in enumerate(self.agents):
