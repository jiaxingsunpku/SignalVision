...
class IPPO_pfrl(RLAgent):
    ...
    def get_phase(self):
        # SUMO 的 current_phase 在黄灯过渡时会返回黄灯相位索引（可能 >= action_space.n），
        # virtual_phase 始终是上一次执行的绿灯相位索引，永远在 [0, n_green_phases) 内。
        phase = np.array([self.inter.virtual_phase], dtype=np.int8)
        return phase
    ...
    def get_action_prob(self, ob, phase):
        """返回当前动作概率分布，供 trainer 记录用。PPO 内部已处理，此处返回均匀分布占位。"""
    ...
    def train(self):
        try:
            ...
            # pfrl PPO statistics list order may vary; find 'value_loss' or fall back to index
            for name, val in stats:
                if 'loss' in name.lower():
                    return float(val)
            return float(stats[0][1]) if stats else 0.0
        except Exception:
            return 0.0
