...
class MADDPGAgent(RLAgent):
    ...
    def sample(self):
        # 各路口相位数不同，必须按各自的 action_space.n 单独采样，不能用同一个上界
        return [np.random.randint(0, ag.action_space.n) for ag in self.agents]

    def get_action_prob(self, obs, phase):
        self.get_action(obs, phase)
        # 各路口相位数可能不同，数组形状不一致，不能 stack，直接返回列表
        return self.prob
    ...
    def train(self):
        ...
        # 每个 agent 的 actor/critic 独立更新，避免梯度在多次 backward 中累积 N 倍
        for idx, ag in enumerate(self.agents):
            # --- critic update ---
            ...
            # --- actor update：每个 agent 独立清零并立即 step ---
            ag.actor.optimizer.zero_grad()
            actor_loss = ag.critic.forward(states, mu).flatten()
            actor_loss = torch.mean(torch.mul(-1, actor_loss))
            actor_loss.backward(retain_graph=True)
            clip_grad_norm_(ag.actor.parameters(), ag.grad_clip)
            ag.actor.optimizer.step()
