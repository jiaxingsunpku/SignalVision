...
class PPOAgent(RLAgent):
    def __init__(self, world, rank):
        ...
        # generate() may pad short arrays (len==2→4, len==3→4), so use the actual
        # output length rather than the ob_length attribute to build the network.
        actual_ob_len = len(self.ob_generator.generate())
        if self.phase:
            if self.one_hot:
                self.ob_length = actual_ob_len + len(self.world.id2intersection[inter_id].phases)
            else:
                self.ob_length = actual_ob_len + 1
        else:
            self.ob_length = actual_ob_len
        ...
        # self.model points to the actor network (used by get_action / save_model / load_model)
        self.model = self.actor.model
        self.target_model = self.actor.model
        self.criterion = nn.MSELoss(reduction='mean')
        actor_params = list(self.actor.model.parameters())
        critic_params = list(self.critic.model.parameters())
        self.optimizer = optim.RMSprop(actor_params + critic_params,
    ...
    def load_model(self, e):
        ...
        self.actor.model.load_state_dict(torch.load(model_name))
        self.model = self.actor.model
        self.target_model = self.actor.model
    ...
    def save_model(self, e):
        ...
        torch.save(self.actor.model.state_dict(), model_name)
